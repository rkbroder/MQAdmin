...
function getPage() {
                         var url = "/MQSeries/Mq84/copyright.htm";
			if(resolutionIsRight()) {
window.open(url,null,"resizable=no,toolbar=no,scrollbars=no,width=766,height=517,screenX=0,screenY=0,dialogTop=0,dialogLeft=0");
			} else {
...
if(is.ns4) {
	//
        // To generate APPLET tag for HotMedia Audio Player
        //
        if (navigator.appName=="Netscape" && navigator.appVersion.indexOf("OS/2")!=-1)
        {
        //
        // OS/2 Netscape 
        //
        message = '<APPLET code="HotAudioLite.class"'+
               ' name="hotaudio"'+
               ' width=250'+
               ' height=50'+
               ' MAYSCRIPT>';
        ...
        {
        //
        // Netscape Navigator version 3.x (uncompressed ZIP)
        //
        message = '<APPLET code="HotAudioLite.class"'+
               ' archive="hotaudio3x.zip"'+
               ' name="hotaudio"'+
               ' width=250'+
               ' height=50'+
               ' MAYSCRIPT>';
       ...
       {
       message = '<APPLET code="HotAudioLite.class"'+
               ' archive="hotaudio.zip"'+
               ' name="hotaudio"'+
               ' width=250'+
               ' height=50'+
               ' MAYSCRIPT>' +
               ' <PARAM NAME="cabbase" VALUE="hotaudio.cab">';
       }
       message = message + '<Param name="src" value="media/intro.hal">' +
                   '<Param name="autostart" value ="True">' +
                   '<Param name="bkcolor" value="FFCC66">' +
                   '<Param name="showslider" value ="false">' +
                   '<Param name="showplay" value="false">' +
                   '<Param name="showstop" value ="false">' +
                   '</applet>';
