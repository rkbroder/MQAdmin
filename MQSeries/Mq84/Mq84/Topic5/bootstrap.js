...
for(var i=0;i<_Pb.length;i++) { if(_Pb[i]){ var _Wb=_Xb(url,_Ob[i]); if(_Tb=="fullscreen")return _Yb(_Wb,"id66a3e72b");
else if(_Tb=="inplace")return _Zb(_Wb,_Vb); else if(_Tb=="window")return _0b(_Wb,"id66a3e72b",800,600,false);
...
else if(_Ub=="window")return window.open(_Wb,"id66a3e72b","directories=no,location=no,menubar=no,resizable=yes,"+"scrollbars=no,status=no,toolbar=no");
