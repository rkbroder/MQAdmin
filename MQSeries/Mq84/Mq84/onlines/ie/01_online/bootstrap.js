...
for(var i=0;i<_Pb.length;i++) { if(_Pb[i]){ var _Wb=_Xb(url,_Ob[i]); if(_Tb=="fullscreen")return _Yb(_Wb,"id3c22387d");
else if(_Tb=="inplace")return _Zb(_Wb,_Vb); else if(_Tb=="window")return _0b(_Wb,"id3c22387d",800,670,true);
...
else if(_Ub=="window")return window.open(_Wb,"id3c22387d","directories=no,location=no,menubar=no,resizable=yes,"+"scrollbars=yes,status=yes,toolbar=no");
